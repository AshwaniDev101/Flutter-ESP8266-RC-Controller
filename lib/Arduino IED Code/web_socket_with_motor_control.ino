...
// --- WiFi Configuration ---
// Replace these placeholders with your local Wi-Fi credentials in a local-only config file.
// Never push real credentials to a public repo.
...
// We expect incoming messages to be JSON objects with keys like BLF, BLB, BRF, BRB, LS, RS.
void webSocketEvent(uint8_t clientID, WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
